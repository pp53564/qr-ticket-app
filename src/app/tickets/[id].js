...
import { useParams, useRouter } from 'next/navigation'; // Use useParams in app directory

const TicketDetailPage = () => {
  const router = useRouter();
  const { user, isLoading } = useUser();
  const { id } = useParams(); // retrieve id from useParams
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <p className="mt-4">Logged in as: {user.name}</p> {/* Display the user's name */}
        <button
